...
- KNOWN: `cargo deny check bans` still flags `openssl` (banned in favor of rustls). It is pulled transitively via `alloy` (`full` feature) and `jsonschema` (`resolve-http`) through `reqwest`'s default `native-tls`. Migrating those to `rustls` is the remaining deliberate fix (it changes the live L2-RPC TLS backend and needs runtime validation).
