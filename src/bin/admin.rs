...
async fn main() -> anyhow::Result<()> {
    ...
    match command.as_str() {
        "commands" => cmd_commands(args).await,
        ...
        "list-agent-keys" => cmd_list_agent_keys(args).await,
        "rotate-keys" => cmd_rotate_keys(args).await,
        "ves-commit-and-anchor" => cmd_ves_commit_and_anchor(args).await,
        other => {
...
/// `commands` subcommand.
async fn cmd_commands(_args: VecDeque<String>) -> anyhow::Result<()> {
    println!("COMMANDS:");
    print_available_commands();
    Ok(())
}
...
/// `migrate` subcommand.
async fn cmd_migrate(mut args: VecDeque<String>) -> anyhow::Result<()> {
...
/// `reencrypt-events` subcommand.
async fn cmd_reencrypt_events(mut args: VecDeque<String>) -> anyhow::Result<()> {
    let mut database_url: Option<String> = None;
    ...
    let payload_encryption = PayloadEncryption::from_env_with_mode(PayloadEncryptionMode::Optional)
    ...
    loop {
        ...
        for row in rows {
            ...
            if is_encrypted
                && !force
                && payload_encryption
                    .decrypt_payload_with_current_key(&row.tenant_id, &aad, &row.payload_encrypted)
                    .await
                    .is_ok()
            {
...
/// `reencrypt-ves-validity-proofs` subcommand.
async fn cmd_reencrypt_ves_validity_proofs(mut args: VecDeque<String>) -> anyhow::Result<()> {
    let mut database_url: Option<String> = None;
    let mut tenant_id: Option<Uuid> = None;
    let mut store_id: Option<Uuid> = None;
    ...
    loop {
        ...
        for row in rows {
            ...
            would_update += 1;
            if let Some(tx) = tx.as_mut() {
                sqlx::query("UPDATE ves_validity_proofs SET proof = $1 WHERE proof_id = $2")
                    .bind(new_ciphertext)
                    .bind(row.proof_id)
                    .execute(&mut **tx)
                    .await?;
...
/// `reencrypt-ves-compliance-proofs` subcommand.
async fn cmd_reencrypt_ves_compliance_proofs(mut args: VecDeque<String>) -> anyhow::Result<()> {
    let mut database_url: Option<String> = None;
    let mut tenant_id: Option<Uuid> = None;
    let mut store_id: Option<Uuid> = None;
    let mut batch_size: usize = 200;
    ...
    loop {
        ...
        for row in rows {
            ...
            would_update += 1;
            if let Some(tx) = tx.as_mut() {
                sqlx::query("UPDATE ves_compliance_proofs SET proof = $1 WHERE proof_id = $2")
                    .bind(new_ciphertext)
                    .bind(row.proof_id)
                    .execute(&mut **tx)
                    .await?;
...
/// `backfill-ves-state-roots` subcommand.
async fn cmd_backfill_ves_state_roots(mut args: VecDeque<String>) -> anyhow::Result<()> {
    let mut database_url: Option<String> = None;
    let mut tenant_id: Option<Uuid> = None;
    let mut store_id: Option<Uuid> = None;
    let mut dry_run = false;
    ...
    for (tenant_id, store_id) in streams {
        ...
        if dry_run {
            ...
            println!("ok: stream={tenant_id}/{store_id} scanned={scanned} updated={updated}");
        }
    ...
    if dry_run {
        ...
        println!("ok: streams={total_streams} scanned={total_scanned} updated={total_updated}");
    }
...
async fn cmd_verify_proof(mut args: VecDeque<String>) -> anyhow::Result<()> {
    ...
    let merkle_root = merkle_root.ok_or_else(|| anyhow::anyhow!("--merkle-root is required"))?;
    let proof_path = proof_path.ok_or_else(|| anyhow::anyhow!("--proof-path is required"))?;
    let leaf_index = leaf_index.ok_or_else(|| anyhow::anyhow!("--leaf-index is required"))?;

    let leaf_bytes =
        hex::decode(&leaf_hash).map_err(|_| anyhow::anyhow!("invalid hex for --leaf-hash"))?;
    let leaf: Hash256 = leaf_bytes
        .try_into()
        .map_err(|_| anyhow::anyhow!("--leaf-hash must be 32 bytes"))?;

    let root_bytes =
        hex::decode(&merkle_root).map_err(|_| anyhow::anyhow!("invalid hex for --merkle-root"))?;
...
/// `export-events` subcommand.
async fn cmd_export_events(mut args: VecDeque<String>) -> anyhow::Result<()> {
    use std::io::Write;
...
/// `list-agent-keys` subcommand.
async fn cmd_list_agent_keys(mut args: VecDeque<String>) -> anyhow::Result<()> {
    let mut database_url: Option<String> = None;
    let mut tenant_id: Option<Uuid> = None;
...
/// `rotate-keys` subcommand.
async fn cmd_rotate_keys(mut args: VecDeque<String>) -> anyhow::Result<()> {
    let mut database_url: Option<String> = None;
    let mut tenant_id: Option<Uuid> = None;
    let mut agent_id: Option<Uuid> = None;
    ...
    let public_key_bytes =
        hex::decode(&public_key).map_err(|_| anyhow::anyhow!("invalid hex for --public-key"))?;
...
/// `ves-commit-and-anchor` subcommand.
async fn cmd_ves_commit_and_anchor(mut args: VecDeque<String>) -> anyhow::Result<()> {
    let mut database_url: Option<String> = None;
    let mut tenant_id: Option<Uuid> = None;
    let mut store_id: Option<Uuid> = None;
