...
async fn test_commitment_chain_verification(
    ...
    assert_eq!(
        commitment2["prev_state_root"], commitment1["new_state_root"],
...
async fn test_database_failover_and_recovery(
    ...
    let count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM ves_events WHERE sequence_number >= 25")
            .fetch_one(&pool)
            .await?;
...
async fn test_x402_batch_settlement_flow(pool: PgPool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let settled_count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM x402_payment_intents WHERE status = 'Settled'")
            .fetch_one(&pool)
            .await?;
...
async fn test_schema_validation_modes(pool: PgPool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test valid event
    let valid_event = create_event_with_payload(
        &tenant_id,
        json!({
            "amount": 99.99,
    ...
    let invalid_event1 = create_event_with_payload(
        &tenant_id,
        json!({
            "amount": -50.00,
            "customer_id": "CUS-12345"
    ...
    let invalid_event2 = create_event_with_payload(
        &tenant_id,
        json!({
            "amount": 99.99
        }),
...
async fn test_compliance_proof_workflow(pool: PgPool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        inputs.get("amount").is_some(),
        "Should expose amount in public inputs"
    );
    assert!(
        inputs
...
struct SubmissionReceipt {
    event_id: uuid::Uuid,
    // Sequence number is not used in this test harness.
}
...
) -> Result<SubmissionReceipt, sqlx::Error> {
...
async fn create_commitment(
    _pool: &PgPool,
    _request: CreateCommitmentRequest,
...
async fn submit_payment_intent(
    _pool: &PgPool,
...
async fn ingest_event_with_auth(
    _pool: &PgPool,
    event: VesEventEnvelope,
    _api_key: &str,
) -> Result<SubmissionReceipt, Box<dyn std::error::Error>> {
    ingest_event(_pool, event).await.map_err(|e| e.into())
}

async fn generate_test_api_key(
    _pool: &PgPool,
    _tenant_id: &TenantId,
    _limit: u64,
...
async fn get_compliance_inputs(
    _pool: &PgPool,
    _event_id: &uuid::Uuid,
) -> Result<serde_json::Value, sqlx::Error> {
    Ok(json!({
...
fn create_mock_stark_proof(inputs: &serde_json::Value) -> serde_json::Value {
    json!({
        ...
        "proof_bytes": STANDARD.encode(vec![0u8; 100_000]),
